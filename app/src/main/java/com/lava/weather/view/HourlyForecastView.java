 ...
 * 12 * 12 = 144dp
 *
 * Created by Shuo.Wang on 2016/12/19.
 */
public class HourlyForecastView extends View {
    ...
    private ArrayList<Weather.HourlyForecastBean> forecastList;
    ...
    public void setData(Weather weather) {
        ...
        if (this.forecastList == weather.getHourly_forecast()) {

            invalidate();
            return;
        }
        try {
            final ArrayList<Weather.HourlyForecastBean> w_hourlyForecast = (ArrayList<Weather.HourlyForecastBean>) weather.getHourly_forecast();
            if (w_hourlyForecast.size() == 0) {// 有可能为空
                return;
            }
            if (!WeatherUtil.isToday(w_hourlyForecast.get(0).getDate())) {// 不是今天的数据
                Log.d("wangshuo","!WeatherUtil.isToday-->"+w_hourlyForecast.get(0).getDate());
                return;
            }
            ...
            for (int i = 0; i < forecastList.size(); i++) {
                Weather.HourlyForecastBean forecast = forecastList.get(i);
